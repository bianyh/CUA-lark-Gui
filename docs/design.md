...
- 日历创建、文档编辑等流程可能打开独立子窗口或模态窗口。
- Windows 执行器不再只截取标题命中 `飞书` 的第一个窗口，而是构建飞书窗口上下文。
- 上下文包含主窗口、当前活动子窗口，以及与主窗口重叠且疑似相关的子窗口。
- 截图区域是这些窗口的联合区域。
- `Observation.ui_hints.window_candidates` 会记录每个候选窗口的 `title`、`role`、`active`、`region`、`relative_region`。
- 模型坐标以联合截图为坐标系，执行器用最近一次截图区域作为执行坐标原点。
